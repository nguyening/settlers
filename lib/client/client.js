...
var Log = function () {
    var logTypes = ['INFO', 'SUCCESS', 'WARN', 'ERROR', 'CHAT'];
    this.log = function (code, message) {
        ...
        if(code == -1) {
          ...
          $('<p><span class="code '+logTypes[code]+'">['+logTypes[code]+']</span> '+message+'</p>').appendTo(logarea);
...
var Client = function () {
    ...
    var redraw = function () {
        ...
        if(state.isMyTurn() && !state.getRoll() && state.getRound() >= 2)
            $('#roll').removeAttr('disabled');
        if(state.isMyTurn() && ((state.getRoll() && state.isBaronState(0)) || state.getRound() < 2))
    ...
    socket.on('players', function (data) {
        ...
        if(data.reason == 'DROP')
            log.log(0, 'Player '+(parseInt(data.player_num)+1)+' ('+data.player+') has left the game.');
        else if(data.reason == 'NEW')
            log.log(0, data.player+' has joined as Player '+(parseInt(data.player_num)+1));
        else 
            log.log(0, 'Lobby has been updated.');
    });

    socket.on('roll', function (data) {
        state.setRoll(data.roll);
        redraw();
        log.log(0, 'Player '+(state.getCurrentPlayer()+1)+' has rolled a '+data.roll+'.');
    });
    ...
    socket.on('promptBaronMove', function (data) {
        ...
        redraw();
        log.log(2, 'BARON PHASE: You need to move the baron to a new tile.');
    });
    socket.on('promptBaronSteal', function (data) {
        state.setBaronState(data.baronState);
        state.setRobbablePlayers(data.robbable);
        redraw();
        log.log(2, 'BARON PHASE: Select a player whose settlement(s) are on that tile to steal from.');
    });
    ...
    socket.on('moveBaron', function (data) {
        ...
        redraw();
        log.log(0, 'BARON PHASE: The baron has been moved!');
    });

    socket.on('overflowNotice', function (data) {
        state.setBaronState(data.baronState);
        redraw();
        log.log(2, 'BARON PHASE: You have more than '+OVERFLOW_HAND_SIZE+' resources in your hand. You must discard half ('+Math.floor(state.getHand(state.getMyNum()).length/2)+') of them.');
    });
    socket.on('overflowWait', function (data) {
        ...
        log.log(2, 'BARON PHASE: Waiting on players to discard half of their hands..('+overflows+')');
    });
    ...
    socket.on('gain', function (data) {
        ...
        if(data.action == 'roll')
            log.log(0, 'ROLL PHASE: Resources have been distributed for the roll.');
            ...
            log.log(1, 'You have given yourself resources.');
        else if(data.action == 'trade')
            log.log(1, 'TRADING: You have gained resources from the current player.');
        else if(data.action == 'exchange')
            log.log(1, 'EXCHANGE: You have gained resources from exchanging.');
    });
    ...
    socket.on('deduct', function (data) {
        ...
        if(data.action == 'build')
            log.log(1, 'BUILD PHASE: You have lost resources from building.');
        else if(data.action == 'steal')
            log.log(2, 'BARON PHASE: You have had some of your resources stolen.');
        else if(data.action == 'overflow')
            log.log(1, 'BARON PHASE: You have handed over your overflow resources.');
        else if(data.action == 'trade')
            log.log(1, 'TRADING: You have given your resources to the current player.');
        else if(data.action == 'exchange')
            log.log(1, 'EXCHANGE: You have lost resources from exchanging.');

    });

    socket.on('tradeEnd', function (data) {
        if(data.reason == 'accept')
            log.log(0, 'TRADING: The announced trade has been accepted.');
        else if(data.reason == 'cancel')
            log.log(0, 'TRADING: The announced trade has been cancelled.');
    ...
    socket.on('buildAccept', function (data) {  
        if(data.type != 'dev') {
            ...
            log.log(0, 'BUILD PHASE: Player ' + (state.getCurrentPlayer() + 1) + ' has built a ' + data.type + '@' + data.coords);
        }
        else {
            dev.draw();
        }
    ...
    socket.on('tradeAnnounce', function (data) {
        ...
        trade.draw();
        log.log(0, 'Player '+(state.getCurrentPlayer()+1)+' has announced a trade offer.');
    });
    ...
    socket.on('devGain', function (data) {
        ...
        if(state.isMyTurn())
            log.log(1, 'You have bought a Development Card.');
        else
            log.log(0, 'Player '+(currentPlayer+1)+' has purchased a Development Card.');
    });
    ...
    socket.on('useDev', function (data) {
        ...
        if(state.isMyTurn()) {
            ...
            log.log(0, 'Player '+(currentPlayer+1)+' have activated '+devLabeling[data.devNum]+'.');
        }
    ...
    socket.on('nextTurn', function (data) {
        log.log(0, 'Player ' + (state.getCurrentPlayer()+1) + ' has ended their turn.');
        state.flushDevQueue();
        ...
        if(data.round < 2 && state.isMyTurn()) {
            if(state.getMyNum() == NUM_PLAYERS - 1) 
                log.log(2, 'SETUP PHASE: You are allowed to place two settlements and two roads for free this turn.');
            else
                log.log(2, 'SETUP PHASE: You are allowed to place one settlement and one road for free this turn.');
        }
        else if(data.round >= 2){
            log.log(-1);
            log.log(0, 'Round '+(data.round-1));
        }
    });

    socket.on('chatMsg', function (data) {
        log.log(4, 'Player '+(parseInt(data.author)+1)+': '+data.message);
    });

    socket.on('error', function (reason){
        log.log(3, 'Unable to connect to room.');
        console.log('Unable to connect to Socket.IO', reason);
    });

    socket.on('connect', function (){
        log.log(0, 'Connected to room.');
    });

    socket.on('lobby:ready', function () {
        log.log(0, 'Lobby is now full, game starting.');
        self.start();
        
    });

    socket.on('error:game', function (data) {
        log.log(3, data.reason);
    });
