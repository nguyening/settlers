...
var Client = function () {
    ...
    socket.on('overflowNotice', function (data) {
        state.setBaronState(data.baronState);
        redraw();
        log.log(1, 'BARON PHASE: You have more than '+OVERFLOW_HAND_SIZE+' resources in your hand. You must discard half ('+Math.floor(state.getHand(state.getMyNum).length/2)+') of them.');
    });
