...
LogicalBoard.prototype = {
	...
	handleSocket : function () {
		...
		io.sockets.on('connection', function (socket) {
			...
			socket.on('buildRequest', function (data) {
				...
				var state = lb.state;
				if(!(state.isMyTurn(socket.id) && state.getRoll()))
					return;
			...
			socket.on('exchangeRequest', function (data) {
				var harborAction;
				if(lb.utils.deduct(lb.state.getCurrHand(), data.tradeCards) &&
	...
	useDev : function (devNum) {
		var currentPlayer = this.state.getCurrentPlayer();
		if(this.state.activateDev(currentPlayer, devNum)) {
			if(devNum == 1) {			// knight
			...
			// else if(devNum == 5) { // should activate VPs?
