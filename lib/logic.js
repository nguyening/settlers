...
LogicalBoard.prototype = {
	...
	sendAll : function (data) {
		io.sockets.emit(data.message, data.data);
	},
	...
	initSocket : function () {
		...
		io.sockets.on('connection', function (socket) {
			...
			socket.on('endTurn', function (data) {
				...
				if(round < 1 && lb.state.currentPlayer == NUM_PLAYERS - 1)
					round += 1/NUM_PLAYERS;
				...
				if(round < 1) {
					currentPlayer = ++lb.state.currentPlayer % NUM_PLAYERS;
