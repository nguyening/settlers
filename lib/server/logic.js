...
var Logic = function (width, height, state, room) {   
	...
	this.events.on('roll', function (data) {
		...
		if(state.getRoll()) {
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'You have already rolled!',
				},
			});
			return;
		}
		...
		if(state.getRound() < 2) {
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'You cannot roll the dice before setup.',
				},
			});
			return;
		}
	...
	this.events.on('requestBaronMove', function (data) {
		...
		if(!state.isBaronState(1)) {
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'You cannot place the baron at this time.',
				},
			});
			return;
		}

		if(state.isBaron(data.coords)) {
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'You must place the baron to on a new hex.',
				},
			});
			return;
		}
		...
		var robbable = robbablePlayers();
		if(robbable.length > 0) {
			state.setBaronState(2);
			state.setRobbablePlayers(robbable);
			room.send(data.invoker.seat, {
	...
	this.events.on('requestBaronSteal', function (data) {
		...
		if(!state.isBaronState(2)) {
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'You cannot rob a player at this time.',
				},
			});
			return;
		}
		...
		if(state.isRobbable(data.player_num)) {
			...
			state.giveResources(data.invoker.seat, oppHand[i]);
			oppHand.splice(i, 1);
			state.setHand(data.player_num, oppHand);
			...
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'You cannot rob that player.',
				},
			});	
		}
	});
	...
	this.events.on('overflowResolve', function (data) {
		if(!state.isBaronState(3)) {
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'You cannot discard resources at this time.',
				},
			});
			return;
		}
		...
		if((t=overflowPlayers.indexOf(p)) == -1) {	// no need to discard
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'You do not need to discard resources.',
				},
			});
			return;
		}
		...
		if(newHand && data.cardsDiscard.length >= Math.floor(state.getHand(p).length/2)) {
			...
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'You have not discarded enough resources. You must discard half of your resources.',
				},
			});
			return;
		}
	...
	this.events.on('buildRequest', function (data) {
		...
		if(!state.getRoll() && state.getRound() >= 2) {
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'You must roll before you build!',
				},
			});
			return;
		}
		...
		if(data.type == 'road' &&
			...
			if(!buildable)
				room.send(data.invoker.seat, {
					message: 'error:game',
					data: {
						reason: 'You cannot build that there.',
					},
				});
			else
				room.send(data.invoker.seat, {
					message: 'error:game',
					data: {
						reason: 'You do not have enough resources to build that.',
					},
				});
		}
	});
	...
	this.events.on('tradeAnnounceRequest', function (data) {
		...
		if(newHand && data.wantCards.length > 0) {
			...
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'This is an invalid trade offer.',
				},
			});
		}
	});
	...
	this.events.on('tradeAnnounceCancel', function (data) {
		...
		if(!(state.getTradeCards().length > 0 && state.getWantCards().length > 0)) {
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'There is no trade to cancel.',
				},
			});
			return;
		}
	...
	this.events.on('tradeAccept', function (data) {
		...
		if(state.isMyTurn(data.invoker.seat)) {
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'You cannot accept your own trade.',
				},
			});
			return;
		}
		...
		if(!state.isMyTurn(data.invoker.seat) && 			// current player shouldn't accept their own trade
			...
			if(newCurrHand && newTradHand) {
				...
				room.send(data.invoker.seat, {
					message: 'error:game',
					data: {
						reason: 'You cannot accept this trade.',
					},
				});
			}
		}
			...
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'There has not been an announced trade.',
				},
			});
		}
	});
	...
	this.events.on('exchangeRequest', function (data) {
		...
		if(state.getActiveDev() == 3 &&
			...
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'You cannot exchange without the proper harbor and trade format (2:1, 3:1, 4:1)',
				},
			});
		}
	});
	...
	this.events.on('useDevRequest', function (data) {
		...
		if(state.isMyTurn(data.invoker.seat) && useDev(data.devNum)) {
			...
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {
					reason: 'You cannot use your development card right now.',
				},
			});
		}	
	});
	...
	this.events.on('endTurn', function (data) {
		...
		if(!state.isBaronState(0)) {
			if(state.isBaronState(1)) {
				room.send(data.invoker.seat, {
					message: 'error:game',
					data: {
						reason: 'You must place the baron somewhere.',
					},
				});
			}
				...
				room.send(data.invoker.seat, {
					message: 'error:game',
					data: {
						reason: 'You must rob from a player.',
					},
				});
			}
			else if(state.isBaronState(3)) {
				room.send(data.invoker.seat, {
					message: 'error:game',
					data: {
						reason: 'You must wait for players to discard their resources.',
					},
				});
			}
		...
		if((round < 1 && (settlements[p].length < 1 || roads[p].length < 1)) ||
			(round > 1 && round < 2 && (settlements[p].length < 2 || roads[p].length < 2))) {
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {reason: 'You need to build a settlement and road before you end!'},
			});
			return;
		}
		if(round < 1 && (settlements[p].length < 2 || roads[p].length < 2) && p == NUM_PLAYERS -1) {
			room.send(data.invoker.seat, {
				message: 'error:game',
				data: {reason: 'You need to build two settlements and two roads before you end!'},
			});
			return;
		}
	...
	// 	var currentPlayer = data.invoker.seat,
